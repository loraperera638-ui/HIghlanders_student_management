...
export default function StudentLayout({
  ...
  const navItems = [
    ...
    { name: 'Billing', href: '/student/billing', icon: DollarSign },
        ...
        <aside className={`fixed inset-y-0 left-0 w-68 bg-gradient-to-b from-[#0A1128] to-[#101b3f] text-white min-h-full flex-shrink-0 flex flex-col border-r border-white/5 z-50 transform transition-transform duration-300 lg:relative lg:translate-x-0 ${
          ...
          <div className="p-5 border-t border-white/5 bg-black/10 flex items-center justify-between">
            <div className="flex items-center space-x-2.5">
              <div className="w-9 h-9 bg-primary-wave/10 border border-primary-wave/20 rounded-full flex items-center justify-center text-primary-wave text-sm font-bold shadow-sm">
                S
              </div>
              <div className="leading-tight">
                <p className="text-xs font-semibold text-white">Active Student</p>
                <span className="text-[9px] text-gray-400">Yellow Belt</span>
              </div>
            </div>
          </div>
        </aside>
